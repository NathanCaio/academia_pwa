module.exports = {
    '@magento/pagebuilder/lib/config.js': 'src/pagebuilder/lib/config.js',

    '@magento/venia-ui/lib/components/Header/header.js':
        'src/components/Header/header.jsx',

    '@magento/venia-ui/lib/components/Logo/logo.js':
        'src/components/Logo/logo.jsx',

    '@magento/venia-ui/lib/components/Header/header.module.css':
        'src/components/Header/header.module.css',

    '@magento/venia-ui/lib/components/Header/searchTrigger.js':
        'src/components/Header/searchTrigger.jsx',

    '@magento/venia-ui/lib/components/Header/accountTrigger.js':
        'src/components/Header/accountTrigger.jsx',

    '@magento/venia-ui/lib/components/AccountChip/accountChip.js':
        'src/components/AccountChip/accountChip.jsx',

    '@magento/venia-ui/lib/components/Header/accountTrigger.module.css':
        'src/components/Header/accountTrigger.module.css',

    '@magento/venia-ui/lib/components/AccountChip/accountChip.module.css':
        'src/components/AccountChip/accountChip.module.css',

    '@magento/venia-ui/lib/components/Icon/icon.js':
        'src/components/Icon/icon.jsx',

    '@magento/venia-ui/lib/components/Icon/icon.module.css':
        'src/components/Icon/icon.module.css',

    '@magento/venia-ui/lib/components/Header/searchTrigger.module.css':
        'src/components/Header/searchTrigger.module.css',

    '@magento/venia-ui/lib/components/Header/cartTrigger.js':
        'src/components/Header/cartTrigger.jsx',

    '@magento/venia-ui/lib/components/Header/cartTrigger.module.css':
        'src/components/Header/cartTrigger.module.css',

    '@magento/venia-ui/lib/components/HomePage/homePage.ee.js':
        'src/components/HomePage/homePage.ee.jsx',

    '@magento/venia-ui/lib/components/Footer/footer.js':
        'src/components/Footer/footer.js',

    '@magento/venia-ui/lib/components/Footer/sampleData.js':
        'src/components/Footer/sampleData.js',

    '@magento/venia-ui/lib/components/Footer/footer.module.css':
        'src/components/Footer/footer.module.css'
};
